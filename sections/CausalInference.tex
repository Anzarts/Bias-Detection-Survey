...
In the alternative version of the hiring example, \textsc{Ex3}, the gender, $X$, is included in the input. In an attemp to debias the model, $X$ is modified in the training data. However, $Z$ remains in its original value, so the model learns the same association between $Z$ and $Y$. The direct effect of $X$ on $Y$ is reduced, but the bias persist. In order to achieve a complete bias detection, it is imperative to ensure that the detection method is assessing the total effect and not only the direct effect.
